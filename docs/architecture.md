...
- 面板内 `Tab` / `Shift+Tab` 循环切模式（IME 组合中不拦截）；焦点在 Tab 条时也可用 ←/→，或 `⌘←/→` · `Ctrl+←/→`。`Esc` 先关当前弹层再隐藏面板；`⌘W` / `Ctrl+W` 只隐藏主面板并保持预热。`⌘Enter` 手动触发、`⌘C` 复制结果。**`Esc` 永远有明确含义**（原生约定）。
...
**色彩（一个强调色，锁死；语义色独立）**
- 中性基调：浅/深半透明 tint（`--j-panel-bg`）叠在平台材质上；设置窗用不透明 `--j-solid-bg`（浅 `#f4f6f8` / 深 `#0f141a`）。
...
**形态（一个圆角体系）**
- 面板本体圆角 **12px**：macOS 由 `window-vibrancy` + contentView layer（`PANEL_CORNER_RADIUS`）裁剪；窗口阴影交给 OS（`shadow: true`），CSS 不给 `.shell` 画 `box-shadow`。
- 组件圆角规则唯一：输入框/按钮 8px、错误卡片 10px、chip/标签 pill。**一个体系，全程遵守**。

**材质**
- 主面板毛玻璃由 **OS** 画：macOS `NSVisualEffectMaterial::HudWindow`（`Active`，避免无焦点时发灰），Windows 优先 Acrylic、失败则 Blur。CSS 只铺半透明 tint，让材质透出。
- 设置窗保持不透明，不套 vibrancy。

### 8.3 布局（Raycast 风，但收敛）

```
┌──────────────────────────────────────────────┐  ← 平台窗口（OS 圆角 12px / 阴影 / vibrancy）
│ [ 搜索  选中自动填入 ]                          │  ← 左侧搜索图标；聚焦即全选；无描边
│ [ 翻译 ] [ 语法 ] [ 错题本 ] [ 历史 ]           │  ← segmented pill；方向键 / ⌘←→ 切换
│ ┌──────────────────────────────────────────┐ │
│ │ 翻译: 原文 / 译文 [+复制]                 │ │
